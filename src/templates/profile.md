# User Profile Context — career-ops
# Mahadev Mahesh Maitri | mahadev@maitri.pro | NYC

## Target Roles

| Archetype | Thematic axes | What they buy |
|-----------|---------------|---------------|
| **AI/ML Engineer** | End-to-end pipelines, RAG, LLM fine-tuning, evals | Someone who ships production ML with measurable impact |
| **Agentic / Automation Engineer** | LangGraph, stateful agents, HITL, orchestration | Someone who builds reliable agent systems that cut manual work |
| **AI Platform / LLMOps** | Observability, evals, retrieval, RAG precision, infra | Someone who puts LLMs in production and keeps them reliable |
| **Forward Deployed Engineer** | Client-facing, fast prototyping, enterprise deployment | Someone who delivers AI solutions into customer environments fast |
| **AI Solutions Architect** | AWS Bedrock, EKS, event-driven, distributed systems | Someone who designs and owns end-to-end AI architectures |

## Adaptive Framing

| If the role is... | Emphasize about Mahadev... | Proof point |
|-------------------|---------------------------|-------------|
| AI/ML Engineer | RAG pipeline +25% precision, 0.948 MAP@3 Kaggle, LLM fine-tuning (QLoRA, PEFT) | cv.md + Kaggle project |
| Agentic / Automation | LangGraph stateful agents → 15% reduction in manual doc review; autonomous orchestration workflows | cv.md (Fulcrum Digital) |
| LLMOps / AI Platform | AWS Bedrock migration (60% GPU cost cut), 99.8% uptime EKS for 300+ users, custom vector DB retriever | cv.md (Fulcrum Digital) |
| Forward Deployed | Doxtract-IDP enterprise deployment (92% extraction confidence, 40% less manual entry); fast delivery mindset | cv.md (Fulcrum Digital) |
| Solutions Architect | Distributed Ray clusters, EKS with HPA, event-driven Bedrock architecture | cv.md (Fulcrum Digital) |

## Exit Narrative

Mahadev is an AI Engineer with 3+ years building and shipping end-to-end ML systems in production. At Fulcrum Digital (NYC), he owns the full stack: RAG pipelines, LangGraph agent orchestration, AWS Bedrock deployments, and infra (EKS, Ray). He's the person who both designs the architecture AND fixes it at 2am when it breaks.

**Frame in applications:** "I build AI systems that work in production — not just in demos."

**Transition story:** Coming from enterprise AI (Fulcrum Digital / Optum), targeting companies where AI is the product, not a support function. Wants to work on harder problems with faster feedback loops.

## Superpowers

1. **End-to-end ML pipelines** — from raw data to prod in weeks, not quarters
2. **LangGraph + agentic systems** — stateful, reliable, production-grade
3. **RAG at scale** — custom retrievers, reranking, PGVector, precision tuning
4. **AWS infra for AI** — Bedrock, EKS, Ray, event-driven, cost optimization
5. **LLM fine-tuning** — QLoRA, PEFT, ensemble pipelines (0.948 MAP@3)

## Key Proof Points (read cv.md for exact metrics)

- **RAG precision +25%** — custom vector DB retriever + LLM reranking (Fulcrum Digital)
- **60% GPU cost reduction** — migrated Ollama → AWS Bedrock
- **99.8% uptime** — EKS + HPA for 300+ concurrent users
- **40% less manual data entry** — Doxtract-IDP with 92% extraction confidence
- **15% reduction in manual doc review** — LangGraph autonomous workflows
- **0.948 MAP@3** — Kaggle LLM fine-tuning competition (top leaderboard)
- **80% less manual review** — NLP pipeline for 10k+ eligibility forms (Optum)

## Comp Targets

- Target: $150K–200K total comp (USD)
- Minimum: $120K base
- Location: NYC (open to hybrid/remote; willing to relocate for right role)
- Visa: H-1B valid until 2029, 3-year extension available

**Salary expectations script:**
> "Based on current market for senior AI/ML engineers in NYC, I'm targeting $150–200K TC. Flexible on structure — total package and scope matter more than split."

**When offered below target:**
> "I'm comparing with roles in the $150–180K range. I'm drawn to [company] because [specific reason]. Is there room to get closer to [target]?"

**Geographic discount pushback:**
> "My work is fully output-based — RAG precision improvements and system uptime don't change by timezone."

## Location Policy

- **NYC / hybrid:** Score at face value
- **Remote (US) / Anywhere in US:** Score normally — candidate is open
- **Remote (international / non-US):** Score 2.0 on location dimension (H-1B constraint)

## Negotiation — H-1B Context

- Current employer sponsors H-1B. New employer must be willing to transfer sponsorship.
- "I'm on H-1B through 2029. Sponsorship transfer is a standard process — most tech companies handle it routinely."
- Do NOT mention visa in initial outreach — only if directly asked.

## Cross-cutting Advantage

**"Production AI builder with measurable outcomes."**

Mahadev's differentiator: he ships systems that stay up (99.8%), cost less (−60% GPU spend), and improve over time (RAG precision +25%). He doesn't just prototype — he owns the architecture from design to monitoring. Frame this in every cover letter opening.

## Portfolio

- **GitHub:** github.com/mahadev9 — LLM fine-tuning, F1 RL agent
- **Portfolio:** maitri.pro
- **Kaggle project:** github.com/mahadev9/map-kaggle-competition (0.948 MAP@3)
- **F1 RL project:** github.com/mahadev9/cisc889

When to share: always include GitHub in applications. Highlight Kaggle project for ML-heavy roles; Fulcrum Digital agent work for agentic/FDE roles.
